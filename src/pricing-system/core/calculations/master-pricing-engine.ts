...
export class MasterPricingEngine {
  ...
  private convertRowToConfig(row: PricingConfigRow): PaverPatioConfig {
    return {
      ...
      variables: this.transformVariablesConfig(row.variables_config)
    } as PaverPatioConfig;
  }

  /**
   * Transform raw Supabase variables_config JSONB to full PaverPatioConfig.variables structure
   * Supabase stores only option values, UI needs full metadata (label, type, default, etc.)
   */
  private transformVariablesConfig(rawConfig: any): PaverPatioConfig['variables'] {
    console.log('🔍 [MASTER ENGINE] Raw variables_config from Supabase:', JSON.stringify(rawConfig, null, 2));

    const transformed = {
      excavation: {
        label: 'Excavation & Tearout',
        description: 'Site preparation and demolition requirements',
        tearoutComplexity: {
          label: 'Tearout Complexity',
          description: 'Type of existing surface to remove',
          type: 'select' as const,
          default: 'grass',
          calculationTier: 1 as const,
          effectType: 'time_modifier',
          options: this.transformOptions(rawConfig.excavation?.tearoutComplexity || {})
        },
        equipmentRequired: {
          label: 'Equipment Required',
          description: 'Heavy machinery or attachment needs',
          type: 'select' as const,
          default: 'handTools',
          calculationTier: 2 as const,
          effectType: 'fixed_cost',
          options: this.transformOptions(rawConfig.excavation?.equipmentRequired || {})
        }
      },
      siteAccess: {
        label: 'Site Access',
        description: 'Access difficulty and obstacles',
        accessDifficulty: {
          label: 'Access Difficulty',
          description: 'How difficult is it to access the work site',
          type: 'select' as const,
          default: 'easy',
          calculationTier: 1 as const,
          effectType: 'time_modifier',
          options: this.transformOptions(rawConfig.siteAccess?.accessDifficulty || {})
        },
        obstacleRemoval: {
          label: 'Obstacle Removal',
          description: 'Trees, bushes, or structures to remove',
          type: 'select' as const,
          default: 'none',
          calculationTier: 2 as const,
          effectType: 'fixed_cost',
          options: this.transformOptions(rawConfig.siteAccess?.obstacleRemoval || {})
        }
      },
      materials: {
        label: 'Materials',
        description: 'Paver style and installation complexity',
        paverStyle: {
          label: 'Paver Style',
          description: 'Standard vs premium paver materials',
          type: 'select' as const,
          default: 'standard',
          calculationTier: 2 as const,
          effectType: 'material_multiplier',
          options: this.transformOptions(rawConfig.materials?.paverStyle || {})
        },
        cuttingComplexity: {
          label: 'Cutting Complexity',
          description: 'How much cutting and fitting required',
          type: 'select' as const,
          default: 'minimal',
          calculationTier: 'both' as const,
          effectType: 'composite',
          options: this.transformOptions(rawConfig.materials?.cuttingComplexity || {})
        },
        patternComplexity: {
          label: 'Pattern Complexity',
          description: 'Simple running bond vs intricate patterns',
          type: 'select' as const,
          default: 'minimal',
          calculationTier: 1 as const,
          effectType: 'time_modifier',
          options: this.transformOptions(rawConfig.materials?.patternComplexity || {})
        }
      },
      labor: {
        label: 'Labor',
        description: 'Team size and efficiency',
        teamSize: {
          label: 'Team Size',
          ...
          calculationTier: 1 as const,
          effectType: 'time_modifier',
          options: this.transformOptions(rawConfig.labor?.teamSize || {})
        }
      },
      complexity: {
        label: 'Overall Complexity',
        description: 'Project-wide complexity adjustment',
        overallComplexity: {
          label: 'Overall Complexity',
          description: 'Subjective assessment of overall job difficulty',
          ...
          options: this.transformOptions(rawConfig.complexity?.overallComplexity || {})
        }
      }
    };

    console.log('✅ [MASTER ENGINE] Transformed variables:', JSON.stringify(transformed, null, 2));
    return transformed;
  }

  /**
   * Transform raw option values to PaverPatioOption format
   * Handles both simple values (number) and complex objects ({materialWaste, fixedLaborHours})
   */
  private transformOptions(rawOptions: any): Record<string, any> {
    ...
    for (const [optionKey, optionValue] of Object.entries(rawOptions)) {
      if (typeof optionValue === 'number') {
        ...
        transformed[optionKey] = {
          label: this.formatLabel(optionKey),
          value: optionValue
        };
      } else if (typeof optionValue === 'object' && optionValue !== null) {
        // Complex object (e.g., cutting complexity with materialWaste + fixedLaborHours)
        transformed[optionKey] = {
          label: this.formatLabel(optionKey),
          ...optionValue
        };
  ...
  /**
   * Convert camelCase to Title Case for option labels
   */
