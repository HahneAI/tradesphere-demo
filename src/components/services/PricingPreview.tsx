    ...
    >
      ...
      >
        ...
        <div className="text-sm mt-1" style={{ color: visualConfig.colors.text.secondary }}>
          ${(calculation.total / parseFloat(sqft || '1')).toFixed(2)} per sq ft
        </div>
      </div>
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span style={{ color: visualConfig.colors.text.secondary }}>Base price per sq ft:</span>
              <span style={{ color: visualConfig.colors.text.primary }}>${calculation.basePrice}</span>
            </div>
            ...
            <div className="space-y-1">
              ...
              {Object.entries(calculation.multipliers).map(([key, value]) => (
            ...
            <div className="flex justify-between text-sm border-t pt-2" style={{ borderColor: visualConfig.colors.text.secondary + '40' }}>
              <span style={{ color: visualConfig.colors.text.secondary }}>Subtotal ({sqft} sq ft):</span>
              <span style={{ color: visualConfig.colors.text.primary }}>${calculation.subtotal.toFixed(2)}</span>
            </div>

            {/* Additional Costs */}
            {(calculation.additionalCosts.equipment > 0 || calculation.additionalCosts.obstacles > 0) && (
              <div className="space-y-1">
                <span className="text-xs font-medium" style={{ color: visualConfig.colors.text.secondary }}>
                  Additional Costs:
                </span>
                {calculation.additionalCosts.equipment > 0 && (
                  <div className="flex justify-between text-xs pl-2">
                    <span style={{ color: visualConfig.colors.text.secondary }}>Equipment:</span>
                    <span style={{ color: visualConfig.colors.text.primary }}>
                      +${calculation.additionalCosts.equipment}
                    </span>
                  </div>
                )}
                {calculation.additionalCosts.obstacles > 0 && (
                  <div className="flex justify-between text-xs pl-2">
                    <span style={{ color: visualConfig.colors.text.secondary }}>Obstacles:</span>
                    <span style={{ color: visualConfig.colors.text.primary }}>
                      +${calculation.additionalCosts.obstacles}
                    </span>
                  </div>
                )}
              </div>
            ...
            >
              <span style={{ color: visualConfig.colors.text.primary }}>Total:</span>
              <span style={{ color: visualConfig.colors.primary }}>${calculation.total.toFixed(2)}</span>
            </div>
          </div>
