...
export class PaverPatioIntelligenceService {
  ...
  static async analyzePaverPatioRequest(input: PaverPatioDetectionPrompt): Promise<PaverPatioAnalysisResult> {
    console.log('🧠 PAVER PATIO INTELLIGENCE ANALYSIS START');
    ...
    try {
      ...
      console.log(`  Paver Patio: ${result.isPaverPatio ? '✅ DETECTED' : '❌ NOT DETECTED'}`);
  ...
  static async processVariableConfirmation(
    ...
    console.log(`🔍 PROCESSING VARIABLE CONFIRMATION: ${questionVariable}`);
    ...
    try {
      ...
      console.error('❌ VARIABLE CONFIRMATION FAILED:', error);
...
1. SQUARE FOOTAGE (CRITICAL):
   ...
   - Descriptive: "small patio" ≈ 150, "medium patio" ≈ 250, "large patio" ≈ 400
   - Only extract if explicitly mentioned or calculable

2. TEAROUT COMPLEXITY (HIGH IMPACT):
   - "grass", "sod", "lawn" → 'grass'
   - "concrete", "existing patio", "demo", "removal" → 'concrete'
   - "asphalt", "pavement", "blacktop" → 'asphalt'
   - Only extract if removal type is mentioned

3. ACCESS DIFFICULTY (HIGH IMPACT):
   - "easy access", "driveway access", "open area" → 'easy'
   - "tight", "narrow gate", "backyard", "moderate access" → 'moderate'
   - "very tight", "hand carry", "difficult access", "no equipment access" → 'difficult'
   - Only extract if access conditions are described

4. TEAM SIZE PREFERENCE (HIGH IMPACT):
   - "small crew", "2-person", "minimal crew" → 'twoPerson'
   - "full crew", "3+ people", "standard crew", "crew of 3" → 'threePlus'
   - Only extract if team size preferences mentioned

5. CUTTING COMPLEXITY (MEDIUM IMPACT):
   - "straight edges", "simple", "rectangular", "square" → 'minimal'
   - "some curves", "moderate cutting", "angled edges" → 'moderate'
   - "complex shapes", "lots of cutting", "intricate design" → 'complex'
   - Only extract if shape/cutting complexity mentioned

6. EQUIPMENT REQUIRED (MEDIUM IMPACT):
   - "hand tools", "manual work", "small job" → 'handTools'
   - "jackhammer", "demo equipment", "pneumatic tools" → 'attachments'
   - "small excavator", "bobcat", "light machinery" → 'lightMachinery'
   - "excavator", "heavy equipment" → 'heavyMachinery'
   - Only extract if equipment needs mentioned

7. PAVER STYLE (MEDIUM IMPACT):
   - "basic", "standard", "budget", "economy grade" → 'economy'
   - "premium", "high-quality", "designer", "natural stone" → 'premium'
   - Only extract if quality preferences mentioned

8. PATTERN COMPLEXITY (LOWER IMPACT):
   - "simple pattern", "running bond", "basic layout" → 'minimal'
   - "herringbone", "pattern work", "decorative" → 'some'
   - "complex patterns", "multiple patterns", "intricate design" → 'extensive'
   - Only extract if pattern preferences mentioned

9. OBSTACLE REMOVAL (LOWER IMPACT):
   - "clear area", "no obstacles" → 'none'
   - "few shrubs", "small plants", "minor obstacles" → 'minor'
   - "trees", "large shrubs", "structures", "major obstacles" → 'major'
...
ANALYZE THE MESSAGE:`;
  ...
  private static async extractVariableFromResponse(userResponse: string, variableName: string): Promise<any> {
    ...
    try {
      ...
      console.log(`🔍 VARIABLE EXTRACTION: ${variableName} = ${result.value} (confidence: ${result.confidence})`);
  ...
  private static buildExtractionPrompt(userResponse: string, variableName: string): string {
    const extractionRules = {
      squareFootage: {
        ...
        examples: "'about 200' → 200, '15x20' → 300, 'medium size' → 250"
      },
      tearoutComplexity: {
        description: "Extract what needs to be removed",
        validValues: "'grass', 'concrete', 'asphalt'",
        examples: "'concrete' → 'concrete', 'existing stuff' → 'concrete', 'just grass' → 'grass'"
      },
      accessDifficulty: {
        description: "Extract access difficulty level",
        validValues: "'easy', 'moderate', 'difficult'",
        examples: "'tight gate' → 'difficult', 'drive right up' → 'easy', 'some maneuvering' → 'moderate'"
      },
      teamSize: {
        description: "Extract preferred team size",
        validValues: "'twoPerson', 'threePlus'",
        examples: "'small crew' → 'twoPerson', 'full team' → 'threePlus', 'quick job' → 'twoPerson'"
      },
      cuttingComplexity: {
      ...
      equipmentRequired: {
        ...
        examples: "'hand tools' → 'handTools', 'jackhammer' → 'attachments', 'excavator' → 'lightMachinery'"
      },
      paverStyle: {
        description: "Extract paver quality preference",
        validValues: "'economy', 'premium'",
        examples: "'basic pavers' → 'economy', 'high quality' → 'premium', 'standard' → 'economy'"
      },
      patternComplexity: {
        description: "Extract pattern complexity",
        validValues: "'minimal', 'some', 'extensive'",
        examples: "'simple pattern' → 'minimal', 'herringbone' → 'some', 'complex design' → 'extensive'"
      },
      obstacleRemoval: {
        description: "Extract obstacle removal needs",
        validValues: "'none', 'minor', 'major'",
        examples: "'clear area' → 'none', 'few shrubs' → 'minor', 'large tree' → 'major'"
      },
      overallComplexity: {
...
EXTRACT THE VALUE:`;
  ...
  private static getFallbackAnalysis(userMessage: string): PaverPatioAnalysisResult {
    console.log('🔄 FALLBACK: Using basic rule-based analysis');
