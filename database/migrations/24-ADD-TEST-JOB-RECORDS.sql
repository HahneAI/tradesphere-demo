-- Migration: Add Test Job Records
-- Purpose: Create 4 realistic job records with services for testing the Jobs UI
-- Date: October 24, 2025
--
-- IMPORTANT: This migration uses CTEs to work in Supabase SQL Editor
-- Run this entire script as one query in Supabase SQL Editor

-- Variables (using actual IDs from database)
-- Company ID: 08f0827a-608f-485a-a19f-e0c55ecf6484
-- Customer IDs:
--   Sarah Johnson: 208a273a-bc07-41df-bc46-497e2eca2af0 (gets 2 jobs)
--   Michael Chen: 5d4c0f52-a321-427a-9130-52b28bcf2c7f (gets 1 job)
--   Emily Rodriguez: 235fc55a-adce-4569-99f2-521abaa655b3 (gets 1 job)
-- User ID (Devon): 50dfad12-a6bc-42cd-a77a-1679fb9619a1
-- Service Config IDs:
--   Paver Patio: 25b669b3-cd3f-4541-8c31-57244f38742e
--   Excavation: f06533a1-9fa2-4bdd-93d7-cbabcef4ce46

-- ============================================================================
-- ALL JOBS AND RELATED DATA IN ONE TRANSACTION
-- ============================================================================
...
job_1 AS (
  INSERT INTO ops_jobs (
    ...
    5, -- Normal priority
    38650.00,
    '2025-02-14', -- Quote valid for 30 days
...
INSERT INTO ops_jobs (
  ...
  '2025-01-27',
  '2025-01-28',
  8, -- High priority (scheduled soon)
  12450.00,
  NOW() - INTERVAL '5 days',
...
INSERT INTO ops_jobs (
  ...
  '2025-01-26',
  NOW() - INTERVAL '3 days',
  10, -- Urgent (commercial project with deadline)
  89750.00,
  NULL, -- Will be calculated when completed
  NULL,
  NULL,
...
INSERT INTO ops_jobs (
  ...
  5, -- Normal priority
  16800.00,
  16250.00, -- Came in slightly under budget
  6200.00,
  8950.00,
...
-- Job Summary:
-- JOB-2025-001: Sarah Johnson - Backyard Paver Patio (QUOTE) - $38,650
--   - 2 services: Paver installation + Excavation
--   - 1 note: Initial discussion
--
-- JOB-2025-002: Sarah Johnson - Front Walkway (SCHEDULED) - $12,450
--   - 2 services: Walkway installation + Concrete removal
--   - 2 notes: Quote approved + Scheduling
--
-- JOB-2025-003: Michael Chen - Commercial Courtyard (IN PROGRESS) - $89,750
--   - 2 services: Commercial pavers + Site prep
--   - 2 notes: Kickoff + Progress update
--
-- JOB-2025-004: Emily Rodriguez - Driveway Extension (COMPLETED) - $16,250
--   - 2 services: Driveway pavers + Base prep (both marked complete)
--   - 3 notes: Sourcing + Completion + Customer feedback

-- Verification Queries:
-- SELECT job_number, title, status, estimated_total FROM ops_jobs ORDER BY job_number;
-- SELECT j.job_number, c.customer_name, j.status FROM ops_jobs j JOIN crm_customers c ON j.customer_id = c.id;
-- SELECT j.job_number, COUNT(s.id) as service_count FROM ops_jobs j LEFT JOIN ops_job_services s ON j.id = s.job_id GROUP BY j.job_number;
-- SELECT j.job_number, COUNT(n.id) as note_count FROM ops_jobs j LEFT JOIN ops_job_notes n ON j.id = n.job_id GROUP BY j.job_number;
